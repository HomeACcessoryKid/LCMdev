...
- create/update the file versions1/latest-pre-release without new-line and setup 1.2.5 version folder
```
mkdir versions1/1.2.5v
echo -n 1.2.5 > versions1/1.2.5v/latest-pre-release
cp versions1/certs.sector* versions1/1.2.5v
cp versions1/public*key*   versions1/1.2.5v
```
- set local.mk to the ota-main program
```
make -j6 rebuild OTAVERSION=1.2.5
mv firmware/otamain.bin versions1/1.2.5v
```
- set local.mk back to ota-boot program
```
make -j6 rebuild OTAVERSION=1.2.5
mv firmware/otaboot.bin versions1/1.2.5v
make -j6 rebuild OTAVERSION=1.2.5 OTABETA=1
cp firmware/otaboot.bin versions1/1.2.5v/otabootbeta.bin
```
- remove the older version files
#
- commit this as version 1.2.5  
- set up a new github release 1.2.5 as a pre-release using the just commited master...  
...
```
make flash OTAVERSION=1.2.5 OTABETA=1
```
...
```
openssl sha384 versions1/1.2.5v/otamain.bin
xxd versions1/1.2.5v/otamain.bin.sig
```

- upload the file versions1/1.2.5v/latest-pre-release to the 'latest release' assets on github
...
```
cp  versions1/1.2.5v/public-1.key.sig versions1
```
...
```
cp  versions1/1.2.5v/certs.sector.sig versions1
```
